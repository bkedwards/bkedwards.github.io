...
export class ParticlebackgroundComponent implements AfterViewInit, OnDestroy {
  ...
  private initThree() {
    ...
    this.pointsMaterial = new THREE.ShaderMaterial({
      ...
      void main() { 
        ...
        gl_PointSize = (step(1.0 - (1.0 / uFov), position.x)) * vDistance * uBlur * 3.0;
        vColor = color;
      }`,
  ...
  private animate = () => {
    ...
    // Step 1: run simulation pass to update position texture
    this.renderer.setRenderTarget(this.simRenderTarget);
    this.renderer.render(this.simScene, this.simCamera);
    this.renderer.setRenderTarget(null);

    // Step 2: draw points using updated positions texture
